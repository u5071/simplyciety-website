...
2026 출판 데이터 마케팅 포럼 청중용 페이지. 현재 **비공개**로 운영한다 — 사이트 메뉴·사이트맵에 노출하지 않고 `robots: noindex`를 걸어 두었으며, 필요한 사람에게 링크를 직접 전달한다. 한국어 독자를 전제로 쓴 유일한 페이지이며, 영어 방문자에게는 안내 문구만 보여준다. 다른 발표를 추가할 때는 이 파일을 복사해 새 경로를 만든다. 공개할 페이지라면 `/ceo`의 `TALKS`와 `app/sitemap.ts`에도 등록한다.

## 대표 소개 (`app/ceo/CeoContent.tsx`)

`CAREER`, `TALKS`, `CREDENTIALS`, `IMPACT` 배열만 고치면 된다. 경력은 발표 슬라이드의 4단계(금융 → 데이터·AI → 유통·문화 → 창업)를 기준으로 하며, **재직했던 회사 이름은 표기하지 않는다**. 사이트의 다른 페이지도 같은 기준을 따른다.
