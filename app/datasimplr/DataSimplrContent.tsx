...
function Eyebrow({ children }: { children: React.ReactNode }) {
  return (
    <p data-reveal className="text-[0.65rem] tracking-[0.4em] uppercase text-[#B8965A] mb-6 flex items-center gap-3">
...
export default function DataSimplrContent() {
  ...
  return (
    <div className="noise-bg bg-[#080808] text-[#F0EDE8] min-h-screen overflow-x-hidden">
      <Nav />
      ...
      <section className="relative px-5 md:px-16 pt-32 md:pt-44 pb-20 md:pb-28">
        ...
        />
        <div className="absolute left-1/2 top-0 -translate-x-1/2 w-[90vw] h-[70vh] pointer-events-none" style={{ background: "radial-gradient(ellipse at 50% 0%, rgba(184,150,90,0.16) 0%, transparent 60%)" }} />

        <div className="relative max-w-screen-xl mx-auto">
          <div className="text-center max-w-4xl mx-auto">
            <div className="hero-eyebrow inline-flex flex-wrap items-center justify-center gap-3 mb-10 px-4 py-2 rounded-full" style={{ border: "1px solid rgba(184,150,90,0.35)", background: "rgba(184,150,90,0.06)" }}>
              <span className="w-1.5 h-1.5 rounded-full bg-[#B8965A] animate-pulse" />
              <span className="text-[0.65rem] tracking-[0.2em] uppercase text-[#E0C48E]">{t(`퍼블릭 베타 · ${LAUNCH_LABEL.ko}`, `Public beta · ${LAUNCH_LABEL.en}`)}</span>
              {cd && <span className="text-[0.65rem] tracking-[0.2em] text-[#8A8780]">D-{cd.d}</span>}
            </div>

            <h1 className="hero-title font-extralight tracking-[-0.035em] leading-[1.02] mb-8">
              <span className="block text-[clamp(1rem,1.6vw,1.25rem)] tracking-[0.35em] uppercase text-[#8A8780] mb-6">
              ...
              <span className="block text-[clamp(2.6rem,7vw,6.5rem)] italic text-transparent bg-clip-text" style={{ backgroundImage: "linear-gradient(90deg, #E0C48E, #B8965A 60%, #8A6E3E)" }}>
                {t("AI가 읽는 데이터.", "No data team required.")}
              </span>
            </h1>

            <p className="hero-body text-[#A8A49E] text-base md:text-xl leading-[1.7] font-light max-w-2xl mx-auto mb-10">
              {t(
          ...
          <div className="mt-16 flex flex-col items-center gap-5">
            <p className="text-[0.6rem] tracking-[0.35em] uppercase text-[#5A5A5A]">{t("이런 데이터와 함께 동작합니다", "Works with the data you already have")}</p>
            <div className="flex flex-wrap justify-center gap-x-8 gap-y-3">
              {WORKS_WITH.map((w) => (
                <span key={w} className="text-sm md:text-base font-light text-[#6A6A6A] tracking-wide">{w}</span>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="max-w-screen-xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-14">
          <div className="lg:col-span-5">
            ...
            <h2 data-reveal className="text-[clamp(2rem,3.6vw,3.1rem)] font-extralight leading-[1.15] tracking-tight mb-8 break-keep">
              {lang === "ko" ? (
                <>AI는 누구나 빌려 씁니다.<br /><span className="italic text-[#B8965A]">차이는 데이터에서</span> 납니다.</>
              ) : (
                <>Anyone can rent a model.<br /><span className="italic text-[#B8965A]">Your data</span> is the edge.</>
              )}
            </h2>
            <p data-reveal className="text-[#8A8780] text-base leading-[1.9] font-light">
              {t(
          ...
          <div className="lg:col-span-7">
            <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-x-4 md:gap-x-8 text-[0.6rem] tracking-[0.3em] uppercase mb-4">
              <span className="text-[#5A5A5A]">{t("지금", "Today")}</span>
              <span />
              <span className="text-[#B8965A]">{t("dataSimplr와 함께", "With dataSimplr")}</span>
            </div>
            {COMPARE.map((c, i) => (
              <div key={i} data-reveal className="grid grid-cols-[1fr_auto_1fr] items-center gap-x-4 md:gap-x-8 py-5" style={{ borderTop: "1px solid rgba(255,255,255,0.07)" }}>
                <p className="text-[#6A6A6A] text-sm md:text-base font-light line-through decoration-white/15">{c.before[lang]}</p>
                <span className="text-[#B8965A]">→</span>
                <p className="text-[#E8E4DE] text-sm md:text-base font-light">{c.after[lang]}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 md:py-32 px-5 md:px-16 bg-[#060606]" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="max-w-screen-xl mx-auto">
          <div className="text-center max-w-2xl mx-auto mb-14">
            <p data-reveal className="text-[0.65rem] tracking-[0.4em] uppercase text-[#B8965A] mb-6">How it works</p>
          ...
          <div data-reveal className="relative mb-14 overflow-x-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-white/[0.06]">
            {STEPS.map((st, i) => (
              <div key={st.k} data-reveal data-reveal-delay={String(i + 1)} className="bg-[#060606] p-8 md:p-10 flex flex-col gap-4">
                <div className="flex items-baseline justify-between">
                  <span className="text-[0.6rem] tracking-[0.3em] text-[#B8965A]">{st.k}</span>
                  <span className="text-[0.55rem] tracking-[0.3em] uppercase text-[#5A5A5A]">{st.name}</span>
                </div>
      ...
      <section className="py-24 md:py-36 px-5 md:px-16 relative overflow-hidden" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="max-w-screen-xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-14 items-center">
          ...
          <div className="order-1 lg:order-2">
            ...
            <h2 data-reveal className="text-[clamp(2rem,4.5vw,3.75rem)] font-extralight leading-[1.08] tracking-tight mb-8">
              {lang === "ko" ? (
                <>데이터 모양에 맞는<br /><span className="italic text-[#B8965A]">네 개의 레이어</span></>
              ) : (
                <>Four layers,<br /><span className="italic text-[#B8965A]">one map of meaning</span></>
              )}
            </h2>
            <p data-reveal className="text-[#8A8780] text-base leading-[1.9] font-light mb-10">
              {t(
            ...
            <dl className="grid grid-cols-2 gap-px bg-white/[0.06]">
              {[
                ...
                <div key={k as string} data-reveal className="bg-[#080808] p-5">
                  <dt className="text-[0.6rem] tracking-[0.3em] uppercase text-[#B8965A] mb-2">{k as string}</dt>
                  <dd className="text-sm text-[#D4D0CA] font-light">{(v as S)[lang]}</dd>
                </div>
              ))}
            </dl>
          </div>
        ...
        <div className="max-w-screen-xl mx-auto">
          <div className="max-w-2xl mb-14">
            ...
            <h2 data-reveal className="text-[clamp(2rem,4.5vw,3.75rem)] font-extralight leading-[1.08] tracking-tight">
              {lang === "ko" ? (
                <>데이터 담당자 대신,<br /><span className="italic text-[#B8965A]">믿을 수 있는 에이전트 팀</span></>
              ) : (
                <>Instead of a data hire,<br /><span className="italic text-[#B8965A]">a team of trustworthy agents</span></>
              )}
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
            {/* agents */}
            <div data-reveal className="md:col-span-4 p-7 md:p-9 bg-[#0A0A0A] border border-white/[0.06] flex flex-col gap-6">
              <div className="flex items-center justify-between">
                <h3 className="text-xl font-light">{t("에이전트", "Agents")}</h3>
                <span className="text-[0.6rem] tracking-[0.2em] uppercase text-[#6A6A6A]">human-in-the-loop</span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {AGENTS.map((a) => (
                  <div key={a.name} className="p-4 border border-white/[0.06] bg-white/[0.015] flex flex-col gap-2">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-[#F0EDE8] flex items-center gap-2">
                        <span className="text-[#B8965A]">✦</span>
                        {a.name}
                      </span>
                      <span className="text-[0.55rem] tracking-[0.15em] uppercase text-[#B8965A]/80">{a.gate[lang]}</span>
                    </div>
                    <p className="text-xs text-[#8A8780] leading-relaxed">{a.role[lang]}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* MCP */}
            <div data-reveal data-reveal-delay="1" className="md:col-span-2 p-7 md:p-9 bg-[#0A0A0A] border border-white/[0.06] flex flex-col gap-5">
              ...
              <pre className="mt-auto text-[11px] leading-[1.7] font-mono text-[#A8A49E] p-4 bg-black/40 border border-white/[0.05] overflow-x-auto">
{`{
  "mcpServers": {
    "datasimplr": {
      "url": "https://mcp.…/acme",
      "scope": "read:sales"
            ...
            {/* sources cited */}
            <div data-reveal className="md:col-span-2 p-7 md:p-9 bg-[#0A0A0A] border border-white/[0.06] flex flex-col gap-5">
              ...
              <div className="mt-auto flex flex-col gap-2 text-[11px] font-mono">
                {["orders ⋈ customers", "def: repeat_buyer", "SELECT … GROUP BY sku"].map((x) => (
                  <span key={x} className="px-3 py-1.5 border border-[#B8965A]/25 text-[#E0C48E] bg-[#B8965A]/[0.05] self-start">{x}</span>
                ))}
              </div>
            </div>
            ...
            <div data-reveal data-reveal-delay="1" className="md:col-span-2 p-7 md:p-9 bg-[#0A0A0A] border border-white/[0.06] flex flex-col gap-5">
              ...
              <div className="mt-auto font-mono text-[11px] border border-white/[0.06]">
                {[["name", "Kim ●●"], ["phone", "010-●●●●-12●●"], ["email", "k●●●@●●●.com"]].map(([k, v]) => (
                  <div key={k} className="flex justify-between px-3 py-2 border-b border-white/[0.04] last:border-0">
                    <span className="text-[#6A6A6A]">{k}</span>
                    <span className="text-[#E0C48E]">{v}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* freshness */}
            <div data-reveal data-reveal-delay="2" className="md:col-span-2 p-7 md:p-9 bg-[#0A0A0A] border border-white/[0.06] flex flex-col gap-5">
              ...
              <svg viewBox="0 0 220 60" className="mt-auto w-full h-14" aria-hidden>
                <polyline points="0,44 20,40 40,42 60,30 80,34 100,22 120,26 140,16 160,20 180,10 200,14 220,6" fill="none" stroke="#B8965A" strokeWidth="1.5" />
                ...
                <defs>
                  <linearGradient id="fg" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0" stopColor="#B8965A" stopOpacity=".25" />
                    <stop offset="1" stopColor="#B8965A" stopOpacity="0" />
                  </linearGradient>
                </defs>
                <circle cx="220" cy="6" r="3" fill="#F1DDB0" />
              </svg>
            </div>
          </div>
        </div>
      ...
      <section className="py-24 md:py-32 px-5 md:px-16" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="max-w-4xl mx-auto text-center">
          ...
          <Link data-reveal href="/ceo" className="inline-flex items-center gap-4 mt-10 group">
            <span className="w-11 h-11 rounded-full flex items-center justify-center text-[#B8965A]" style={{ border: "1px solid rgba(184,150,90,0.5)" }}>
              {t("양", "SY")}
            </span>
            <span className="text-left">
              <span className="block text-sm text-[#F0EDE8] group-hover:text-[#B8965A] transition-colors">{t("양성열", "Sungreul Yang")}</span>
              <span className="block text-xs text-[#6A6A6A]">{t("simplyciety 대표 · dataSimplr 메이커", "Founder, simplyciety · maker of dataSimplr")}</span>
            </span>
          </Link>
        </div>
      </section>
      ...
      {/* ─── WAITLIST ─── */}
      <section id="waitlist" className="py-24 md:py-36 px-5 md:px-16 relative overflow-hidden scroll-mt-20" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse at 20% 100%, rgba(184,150,90,0.12) 0%, transparent 55%)" }} />
        <div className="relative max-w-screen-xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-14">
          <div className="lg:col-span-5 flex flex-col">
            ...
            <h2 data-reveal className="text-[clamp(2.5rem,5.5vw,4.75rem)] font-extralight leading-[0.98] tracking-tight mb-10">
              {lang === "ko" ? (
                <>{LAUNCH_LONG.ko.replace("2026년 ", "")},<br /><span className="italic text-[#B8965A]">가장 먼저.</span></>
              ) : (
                <>{LAUNCH_LONG.en.replace(", 2026", "")}.<br /><span className="italic text-[#B8965A]">Be first in.</span></>
              )}
            </h2>

            {cd && (
              <div data-reveal className="grid grid-cols-4 gap-px bg-white/[0.08] mb-10 max-w-md">
                {[
                  [cd.d, t("일", "days")],
                  [cd.h, t("시간", "hrs")],
                  [cd.m, t("분", "min")],
                  ...
                  <div key={l as string} className="bg-[#080808] py-4 text-center">
                    <p className="text-3xl md:text-4xl font-extralight tabular-nums text-[#F0EDE8]">{String(v).padStart(2, "0")}</p>
                    <p className="text-[0.55rem] tracking-[0.25em] uppercase text-[#6A6A6A] mt-1">{l as string}</p>
                  </div>
                ))}
              </div>
            )}
          ...
          <div data-reveal data-reveal-delay="1" className="lg:col-span-7 p-6 md:p-10 bg-[#0A0A0A]/80 border border-white/[0.07] backdrop-blur">
      ...
      <section className="py-24 md:py-32 px-5 md:px-16 bg-[#060606]" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="max-w-screen-xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className="lg:col-span-4">
            <p className="text-[0.65rem] tracking-[0.4em] uppercase text-[#B8965A] mb-6">FAQ</p>
            <p className="text-sm text-[#6A6A6A] font-light leading-relaxed">
              {t("도입·파트너십 문의는 ", "For partnerships or enterprise questions, ")}
              <Link href="/contact?service=datasimplr" className="text-[#B8965A] hover:underline">{t("문의하기", "contact us")}</Link>
              {t("로 남겨주세요.", ".")}
            </p>
          </div>
          <div className="lg:col-span-8 flex flex-col">
            {FAQ.map((item, i) => (
              <details key={i} className="group py-6" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
                ...
                <p className="text-[#8A8780] text-sm leading-[1.9] font-light mt-4 max-w-2xl">{item.a[lang]}</p>
              </details>
            ))}
          </div>
        </div>
