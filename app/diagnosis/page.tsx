...
function Radar({ dims, labels }: { dims: Result["dims"]; labels: string[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} className="w-full max-w-[360px] mx-auto overflow-visible" role="img" aria-label="radar chart">
      {[1, 2, 3, 4].map((lv) => (
        <polygon key={lv} points={poly(() => (R * lv) / 4)} fill="none" stroke="rgb(var(--hairline) / 0.07)" />
      ))}
      {dims.map((_, i) => {
        const [x, y] = pt(i, R);
        return <line key={i} x1={c} y1={c} x2={x} y2={y} stroke="rgb(var(--hairline) / 0.06)" />;
      })}
      <polygon points={poly((i) => (R * dims[i].score) / 4)} fill="rgb(var(--accent-rgb) / 0.18)" stroke="var(--ds-accent)" strokeWidth="1.5" />
      {dims.map((d, i) => {
        const [x, y] = pt(i, (R * d.score) / 4);
        return <circle key={d.id} cx={x} cy={y} r="3.5" fill={d.score <= 2 ? "var(--ds-danger)" : "var(--ds-accent-strong)"} />;
      })}
      {dims.map((d, i) => {
        const [x, y] = pt(i, R + 26);
        const anchor = Math.abs(x - c) < 8 ? "middle" : x > c ? "start" : "end";
        return (
          <text key={d.id} x={x} y={y} textAnchor={anchor} dominantBaseline="middle" fontSize="10.5" fill="var(--ds-text-secondary)">
            {labels[i]} <tspan fill="var(--ds-accent)">{d.score}</tspan>
          </text>
        );
      })}
    </svg>
        ...
        <div className={`mx-auto w-full ${result ? "max-w-screen-xl" : "max-w-3xl"}`}>
          ...
          {result && (
            <div className="ds-fade">
              <div className="flex flex-wrap items-center justify-between gap-4 mb-12">
                <p className="text-[0.6rem] tracking-[0.4em] uppercase text-accent flex items-center gap-3">
                  <span className="w-5 h-px bg-accent" />
                  AI Readiness Report
                </p>
                <button onClick={reset} className="text-[0.6rem] tracking-[0.25em] uppercase text-text-muted hover:text-accent transition-colors">
                  {t("← 다시 진단하기", "← Retake")}
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 mb-16">
                <div className="lg:col-span-5 flex flex-col gap-6">
                  <div className="flex items-end gap-4">
                    <span className="text-[clamp(5rem,12vw,9rem)] font-extralight leading-[0.85] tracking-tight text-accent">{result.score}</span>
                    <span className="text-text-muted text-sm mb-3">/ 100</span>
                  </div>
                  <div className="flex gap-1.5">
                    {(["foundation", "explore", "scale", "native"] as const).map((sid) => (
                      <div key={sid} className="flex-1">
                        <div className="h-1" style={{ background: sid === result.stage ? "var(--ds-accent)" : "rgb(var(--hairline) / 0.08)" }} />
                        <p className="mt-2 text-[0.55rem] tracking-[0.15em] uppercase" style={{ color: sid === result.stage ? "var(--ds-accent)" : "var(--ds-text-muted)" }}>
                          {STAGES[sid].range}
                        </p>
                      </div>
                    ))}
                  ...
                  <p className="text-text-secondary text-base leading-[1.9] font-light">{STAGES[result.stage].desc[lang]}</p>
                </div>
                <div className="lg:col-span-7 flex items-center justify-center p-6 md:p-10" style={{ border: "1px solid rgb(var(--hairline) / 0.06)", background: "var(--ds-elevated)" }}>
              ...
              {/* Priorities */}
              <div className="mb-16">
                <p className="text-[0.6rem] tracking-[0.3em] uppercase text-text-muted mb-6">{t("우선 개선 과제", "Priority actions")}</p>
                {result.gaps.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-border">
                    {result.gaps.map((g, i) => {
                      ...
                      return (
                        <div key={g} className="bg-bg p-7 flex flex-col gap-3">
                          <div className="flex items-center justify-between">
                            <span className="text-[0.6rem] tracking-[0.3em] text-accent">0{i + 1}</span>
                            <span className="text-[0.6rem] text-danger">Level {sc}/4</span>
                          </div>
                          <h3 className="text-xl font-extralight tracking-tight">{d.short[lang]}</h3>
                          <p className="text-text-secondary text-sm leading-[1.85] font-light">{d.action[lang]}</p>
                        </div>
                      );
                    })}
                  </div>
              ...
              <div className={`grid grid-cols-1 ${result.dataSimplrFit ? "lg:grid-cols-2" : ""} gap-4`}>
                {(() => {
                  ...
                  return (
                    <div className="p-8 md:p-10 flex flex-col gap-5" style={{ border: "1px solid rgb(var(--accent-rgb) / 0.3)", background: "rgb(var(--accent-rgb) / 0.03)" }}>
                      <p className="text-[0.6rem] tracking-[0.3em] uppercase text-accent">{t("추천 서비스", "Recommended service")}</p>
                      <div>
                        <h2 className="text-2xl font-light">{svc.name}</h2>
                        <p className="text-text-muted text-sm mt-1">{svc.sub[lang]}</p>
                      </div>
                      <p className="text-text-secondary text-sm leading-[1.85] font-light">{svc.desc[lang]}</p>
                      <p className="text-xs text-text-muted">{t("예상 기간", "Timeline")} · {svc.duration[lang]}</p>
                ...
                {result.dataSimplrFit && (
                  <div className="p-8 md:p-10 flex flex-col gap-5" style={{ border: "1px solid rgb(var(--hairline) / 0.08)", background: "var(--ds-elevated)" }}>
                    <p className="text-[0.6rem] tracking-[0.3em] uppercase text-accent">{t("셀프서비스로 시작하기", "Start self-serve")}</p>
                    <h2 className="text-2xl font-light">
                      data<span className="italic text-accent">Simplr</span>
                    </h2>
                    <p className="text-text-secondary text-sm leading-[1.85] font-light">
                      {t(
              ...
              <p className="mt-10 text-[0.7rem] text-text-muted leading-relaxed">
                {t(
          ...
          {!result && q && (
            <div>
              {step === 0 && (
                <div className="mb-14">
                  <p className="text-[0.6rem] tracking-[0.4em] uppercase text-accent mb-6 flex items-center gap-3">
                    <span className="w-5 h-px bg-accent" />
                  ...
                  <h1 className="text-[clamp(2.2rem,5vw,4rem)] font-extralight leading-[1.08] tracking-tight mb-5">
                    ...
                    <br />
                    <span className="italic text-accent">{t("얼마나 되어 있을까", "for AI?")}</span>
                  </h1>
                  <p className="text-text-muted text-sm md:text-base leading-[1.8] font-light max-w-xl">
                    {t(
              ...
              <div className="flex items-center justify-between mb-4">
                <p className="text-[0.6rem] tracking-[0.25em] uppercase text-text-muted">
                  {q.kind === "dimension" ? q.short[lang] : t("기본 정보", "About you")}
                </p>
                <p className="text-[0.6rem] tracking-[0.2em] text-text-muted">
                  {String(step + 1).padStart(2, "0")} / {String(TOTAL).padStart(2, "0")}
                </p>
              </div>
              <div className="h-px bg-border mb-12 relative">
                <div className="absolute inset-y-0 left-0 bg-accent transition-all duration-500" style={{ width: `${(step / TOTAL) * 100}%` }} />
              </div>
              ...
              <div key={q.id} className="ds-fade">
                ...
                {q.help && <p className="text-text-muted text-sm mb-10">{q.help[lang]}</p>}
                    ...
                    ? q.options.map((opt, i) => {
                        ...
                        return (
                          <button
                            ...
                            style={{
                              border: `1px solid ${active ? "rgb(var(--accent-rgb) / 0.6)" : "rgb(var(--hairline) / 0.08)"}`,
                              background: active ? "rgb(var(--accent-rgb) / 0.07)" : "transparent",
                            }}
                          >
                            <span className="flex gap-0.5 flex-shrink-0" aria-hidden>
                              {[1, 2, 3, 4].map((b) => (
                                <span key={b} className="w-1.5 h-4" style={{ background: b <= v ? (active ? "var(--ds-accent)" : "rgb(var(--accent-rgb) / 0.45)") : "rgb(var(--hairline) / 0.08)" }} />
                              ))}
                            </span>
                            <span className="text-sm md:text-[0.95rem] font-light leading-snug" style={{ color: active ? "var(--ds-text)" : "var(--ds-text-secondary)" }}>
                              {opt[lang]}
                            </span>
                          </button>
                        );
              ...
              <div className="flex items-center justify-between">
                <button
                  ...
                  disabled={step === 0}
                  className="text-[0.6rem] tracking-[0.25em] uppercase text-text-muted hover:text-text transition-colors disabled:opacity-0"
                >
                  ← {t("이전", "Back")}
                </button>
                <p className="text-[0.65rem] text-text-muted">{t("선택하면 다음 질문으로 넘어갑니다", "Selecting an answer moves you forward")}</p>
              </div>
            </div>
          )}
        </div>
