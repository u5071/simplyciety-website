    ...
    <div className="bg-bg text-text font-[var(--font-geist-sans)] min-h-screen">
      ...
      <section className="relative min-h-[55vh] flex flex-col justify-end px-8 md:px-16 pb-20 pt-40 overflow-hidden">
        <div className="absolute top-0 right-0 w-[50vw] h-[60vh] pointer-events-none"
          style={{ background: "radial-gradient(ellipse at top right, rgb(var(--accent-rgb) / 0.04) 0%, transparent 65%)" }} />
        <div className="relative max-w-screen-xl mx-auto w-full">
          ...
          <h1 className="text-[clamp(3rem,8vw,9rem)] font-extralight leading-[0.92] tracking-[-0.03em] mb-8">
            ...
            <span className="italic text-accent">{t("진짜 이야기.", "real stories.")}</span>
          </h1>
          <p className="text-text-muted text-sm font-light tracking-wide max-w-lg">
            {t(
      ...
      {/* POST LIST */}
      <section className="px-8 md:px-16 pb-32" style={{ borderTop: "1px solid rgb(var(--hairline) / 0.06)" }}>
        <div className="max-w-screen-xl mx-auto">
          <div className="flex flex-col divide-y" style={{ borderColor: "rgb(var(--hairline) / 0.04)" }}>
            {posts.map((post, i) => {
              ...
              return (
                <Link
                  ...
                  className="group grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-16 py-10 md:py-14 hover:bg-elevated transition-colors duration-300 -mx-8 md:-mx-16 px-8 md:px-16"
                  style={{ borderColor: "rgb(var(--hairline) / 0.04)" }}
                >
                  {/* Number */}
                  <div className="hidden md:flex md:col-span-1 items-start pt-1">
                    <span className="text-[0.5rem] tracking-[0.2em] text-text-muted">
                  ...
                  <div className="md:col-span-8">
                    <div className="flex items-center gap-4 mb-4">
                      <span
                        ...
                        style={{ border: "1px solid rgb(var(--accent-rgb) / 0.25)", color: "var(--ds-accent)" }}
                      >
                        {cat[lang]}
                      </span>
                      <span className="text-[0.5rem] tracking-[0.15em] uppercase text-text-muted">
                        {t(`${post.readTime}분 읽기`, `${post.readTime} min read`)}
                      </span>
                    </div>
                    <h2 className="text-xl md:text-2xl font-extralight leading-[1.3] tracking-tight text-text group-hover:text-text transition-colors duration-300 mb-3">
                      {lang === "ko" ? post.title : post.title_en}
                    </h2>
                    <p className="text-text-muted text-sm leading-[1.85] font-light line-clamp-2">
                  ...
                  <div className="md:col-span-3 flex md:flex-col md:items-end justify-between md:justify-start gap-4 md:pt-1">
                    <span className="text-[0.5rem] tracking-[0.2em] uppercase text-text-muted">
                      {formatDate(post.date, lang)}
                    </span>
                    <span className="text-accent/30 group-hover:text-accent transition-colors duration-300 text-sm">
                      →
                    </span>
                  </div>
                </Link>
        ...
        <div className="max-w-screen-xl mx-auto flex flex-col md:flex-row items-start md:items-center justify-between gap-10">
          <div>
            ...
            <h2 className="text-[clamp(2rem,4vw,4rem)] font-extralight leading-[1.0] tracking-tight">
              ...
              <span className="italic text-accent">{t("적용하고 싶다면.", "to work.")}</span>
            </h2>
          </div>
          <div className="flex flex-col gap-3 flex-shrink-0">
            ...
            <Link
              href="/contact"
              className="text-[0.6rem] tracking-[0.25em] uppercase text-text-muted hover:text-text-muted transition-colors text-center"
            >
